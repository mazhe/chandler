...
try:
    ...
    if os.path.isfile(DEFAULT_CONFIG_FILE):
        ...
        sys.stderr.write("The configuration file is searched into " + DEFAULT_CONFIG_FILE +
                         " or in the location given by the $CHANDLER_CONF_FILE environement variable\n")
        sys.exit(1)
else:
...
if CACHING:
    ...
    CACHING_RESOURCES_DELAY = config.getint(
        'oarapi', 'caching_resources_delay')
...
resources = get(
    '/resources/details', CACHING_RESOURCES_FILE, CACHING_RESOURCES_DELAY)["items"]
...
if NODES_USAGE_BY_DEFAULT ^ options.toggle_nodes and len(jobs) > 0:
    ...
    for node in natsorted(nodes_usage.keys()):
        ...
        for job in nodes_usage[node]:
            d = time.strftime(
                "%H:%M:%S", time.gmtime(job["start_time"] + job["walltime"] - time.time()))
